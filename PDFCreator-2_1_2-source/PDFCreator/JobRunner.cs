...
{
    /// <summary>
    /// The JobRunner class manages the thread that processes the print jobs. It listens for new jobs in the <see cref="pdfforge.PDFCreator.JobInfoQueue"/> and creates the processing thread if required.
    /// </summary>
    internal class JobRunner
    {
        ...
        private Action ManagePrintJobsAction { get; set; }
        ...
        {
            if (JobInfoQueue.Instance.IsEmpty && !_managePrintJobs)
            {
                return;
            }
        ...
        private void ProcessJob(IJobInfo jobInfo)
        {
            ...
            ConversionWorkflow cw = WorkflowFactory.CreateWorkflow(jobInfo, SettingsHelper.Settings);
