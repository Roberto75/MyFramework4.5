...
{
    /// <summary>
    /// The class ParameterHelper loads parameters that should not be included in the source code of the tests (i.e. passwords and IP addresses for integration tests) from an external text file
    /// </summary>
    public static class ParameterHelper
    {
        ...
        public static string GetPassword(string name)
        {
            ...
            string password = _passwordDictionary[name];
        ...
        private static string FindPasswordFile()
        {
            ...
            {
                string passwordFile = Path.Combine(candidate, "passwords.txt");
                if (File.Exists(passwordFile))
                    return passwordFile;
            }
