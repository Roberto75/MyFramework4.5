...
// ReSharper restore InconsistentNaming
        ...
        public virtual ICollection<string> GetPDFCreatorPrinters()
        {
            ...
            foreach (var printer in printerInfos)
            {
                ...
                if (printer.pDriverName.Equals("PDFCreator"))
        ...
        public bool IsValidPrinterName(string printerName)
        {
            ...
            {
                if (installedPrinter.Equals(printerName))
                    return false;
            }
        ...
        /// <summary>
        /// Function that searches all printers connected to the PDFCreator port and returns the most applicable. This is either the chosen primary printer, 
        /// the default Printer, the one with the name "PDFCreator" or the first one in alphabetical order.
        /// </summary>
        /// <param name="primaryPDFCreatorPrinter">Name of the primary PDFCreator printer</param>
        ...
        public string GetApplicablePDFCreatorPrinter(string primaryPDFCreatorPrinter, string defaultPrinter)
        {
            ...
            foreach (string printer in printers)
            {
                ...
                if (!isDefaultPrinter)
                    if (printer.Equals("PDFCreator", StringComparison.InvariantCultureIgnoreCase))
                        PDFCreatorPrinter = printer;
 
                if (printer.Equals(defaultPrinter, StringComparison.InvariantCultureIgnoreCase))
                {
                    PDFCreatorPrinter = printer;
                    isDefaultPrinter = true;
                }

                if (printer.Equals(primaryPDFCreatorPrinter, StringComparison.InvariantCultureIgnoreCase))
                    return printer;
            }
