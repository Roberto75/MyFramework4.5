...
namespace pdfforge.PDFCreator.Shared.Assistants
{
    public class UacAssistant
    {
        ...
        {
            var explorerText = TranslationHelper.Instance.GetTranslation("Program", "ExplorerIntegration",
                "Create PDF and image files with PDFCreator");

            CallSetupHelper("/FileExtensions=Add /Description=\"" +
                            explorerText + "\"");
        }
        ...
        private bool CallProgramAsAdmin(string path, string arguments)
        {
            ...
            {
                var message = TranslationHelper.Instance.GetTranslation("ApplicationSettingsWindow", "SufficientPermissions",
                    "Operation failed. You probably do not have sufficient permissions.");
                var caption = TranslationHelper.Instance.GetTranslation("ApplicationSettingsWindow", "Error", "Error");
        ...
        private void CallSetupHelper(string arguments)
        {
            ...
            {
                var message = TranslationHelper.Instance.GetFormattedTranslation("Application", "SetupFileMissing",
            ...
            if (Environment.OSVersion.Version.Major <= 5)
            {
                ...
                {
                    var message = TranslationHelper.Instance.GetFormattedTranslation("ApplicationSettingsWindow", "OperationRequiresAdminPrivileges",
                    ...
                    Path.GetFileName(setupHelperPath));
                    var caption = TranslationHelper.Instance.GetTranslation("ApplicationSettingsWindow", "AdminPrivilegesRequired", "Admin privileges required");
