...
namespace pdfforge.PDFCreator.Shared.ViewModels.Wrapper
{
    /// <summary>
    /// SynchronizedCollection is a wrapper that binds an IList and an ObserverableCollection. All changes that happen inside the ObservableCollection will be replicated to the base collection.
        ...
        private void OnObservableCollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            switch (e.Action)
            {
                ...
                case NotifyCollectionChangedAction.Reset:
                    Collection.Clear();
                    foreach (T itm in ObservableCollection)
                        Collection.Add(itm);
                    break;
                default:
