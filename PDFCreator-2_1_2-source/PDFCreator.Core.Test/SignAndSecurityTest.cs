...
namespace PDFCreator.Core.Test
{
    ...
    class SignAndSecurityTest
    {
        ...
        public void SetUp()
        {
            _th = new TestHelper("SigningTest");
        ...
        private void MakeSigningTest()
        {
            ...
            Assert.IsTrue(Math.Abs(ts.TotalHours) < 1, "Timestamp has a difference, bigger than 1h from now");
        ...
        {
            //Testing Opening
            Assert.That(() => { var reader = new PdfReader(_th.Job.OutputFiles[0], Encoding.Default.GetBytes(password)); reader.Close(); },
        ...
        {
            //Testing Opening
            Assert.That(() => { var reader = new PdfReader(_th.Job.OutputFiles[0]); reader.Close();},
                canOpen ? !Throws.Exception.TypeOf<BadPasswordException>() : Throws.Exception.TypeOf<BadPasswordException>());
