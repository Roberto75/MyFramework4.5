...
namespace pdfforge.PDFCreator.Core.Actions
{
    public class FtpAction : IAction, ICheckable
    {
        ...
        public ActionResult ProcessJob(IJob job)
        {
            ...
            try
            {
                ActionResult result = FtpUpload(job);
                return result;
            }
            catch (Exception ex)
            {
        ...
        private ActionResult FtpUpload(IJob job)
        {
            ...
            string fullDirectory = job.TokenReplacer.ReplaceTokens(job.Profile.Ftp.Directory).Trim();
            ...
            string[] directories = fullDirectory.Split(new[] {"/"}, StringSplitOptions.RemoveEmptyEntries);
            ...
            foreach (string file in job.OutputFiles)
            {
                try
                {
                    string targetFile = Path.GetFileNameWithoutExtension(file) + addendum + Path.GetExtension(file);
