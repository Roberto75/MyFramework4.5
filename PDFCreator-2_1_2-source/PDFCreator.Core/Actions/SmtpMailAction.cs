...
namespace pdfforge.PDFCreator.Core.Actions
{
    public class SmtpMailAction : IAction, ICheckable
    {
        ...
        public ActionResult ProcessJob(IJob job)
        {
            ...
            try
            {
                ActionResult result = SendMailOverSmtp(job);
                return result;
            }
            catch (Exception ex)
            {
        ...
        private ActionResult SendMailOverSmtp(IJob job)
        {
            ...
            var mail = new MailMessage(job.Profile.EmailSmtp.Address, job.Profile.EmailSmtp.Recipients.Replace(';', ','));
            ...
            if (job.Profile.EmailSmtp.AddSignature)
            {
                mail.Body += _signatureText;
            }
