...
namespace pdfforge.PDFProcessing
{
    public static class StamperCreator
    {
        ...
        public static PdfStamper CreateStamperAccordingToEncryptionAndSignature(string sourceFile, string destinationFile, ConversionProfile profile)
        {
            PdfStamper stamper;
            ...
            {
                throw new ProcessingException(ex.GetType() + " while creating stamper:" + Environment.NewLine + ex.Message, 25999);
            }

            return stamper;
        }

        private static PdfStamper DoCreateStamperAccordingToEncryptionAndSignature(string sourceFilename, string destinationFilename, ConversionProfile profile)
        {
