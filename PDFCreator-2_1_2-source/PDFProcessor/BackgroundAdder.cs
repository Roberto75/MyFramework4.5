...
namespace pdfforge.PDFProcessing
{
    class BackgroundAdder
    {
        ...
        public static void AddBackground(PdfStamper stamper, ConversionProfile profile)
        {
            ...
            {
                throw new ProcessingException(ex.GetType() + " while addding background:" + Environment.NewLine + ex.Message, 17999);
            }
        }
        ...
        private static void DoAddBackground(PdfStamper stamper, ConversionProfile profile)
        {
            ...
            int nFile = stamper.Reader.NumberOfPages;
            ...
            {
                int backgroundPageNumber = GetBackgroundPageNumber(i, nBackground, profile.BackgroundPage.Repetition,
                ...
                PdfContentByte documentPage = stamper.GetUnderContent(i);
                Rectangle documentPageSize = stamper.Reader.GetPageSize(i);
        ...
        private static void AddPageWithRotationAndScaling(PdfContentByte documentPage, Rectangle documentPageSize, PdfImportedPage backgroundPage, Rectangle backgroundPageSize, int rotation)
        {
            float scaleWidth;
            float scaleHeight;
            float scale;
        ...
        private static int GetBackgroundPageNumber(int currentDocumentPage, int numberOfBackgroundpages, BackgroundRepetition repeat, int startOffset, int endPage)
        {
